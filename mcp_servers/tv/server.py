"""mcp-tv — MCN TV's Business Unit MCP Server (TDD §5.2).

Exposes KB Search + SQL Analytics tools, behind MCN TV's own Authorization
Middleware (ADR-0008 — callable-based, per-unit).
"""
from fastmcp import FastMCP
from fastmcp.exceptions import ToolError
from fastmcp.server.dependencies import get_http_headers

from auth import check_tv_access
...
def _current_auth_context() -> AuthContext:
    ...
    (X-Nova-User et al.), not a verified token. See mcp_servers/tv/CLAUDE.md."""
...
async def kb_search(query: str, top_k: int = 5) -> list[dict]:
    """Search MCN TV's internal knowledge base (SOPs, documentation) for
    content relevant to `query`."""
    if not check_tv_access(_current_auth_context()):
        raise ToolError("Not authorized for MCN TV's data.")
    return await _kb_search(query, top_k)


@mcp.tool
async def sql_analytics(question: str) -> dict:
    """Answer an analytics question about MCN TV's programs, viewership
    ratings, or ad revenue via read-only SQL."""
    if not check_tv_access(_current_auth_context()):
        raise ToolError("Not authorized for MCN TV's data.")
