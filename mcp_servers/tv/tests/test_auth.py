...
def test_allows_caller_claiming_tv_business_unit():
    ctx = AuthContext(user_id="u1", business_units=["tv"], roles=[])
    assert check_tv_access(ctx) is True
...
def test_denies_caller_with_unrelated_business_unit_and_no_allowed_role():
    ctx = AuthContext(user_id="u1", business_units=["plus"], roles=["mcn_plus_employee"])
    assert check_tv_access(ctx) is False
