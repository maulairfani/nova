...
def test_allows_caller_claiming_plus_business_unit():
    ctx = AuthContext(user_id="u1", business_units=["plus"], roles=[])
    assert check_plus_access(ctx) is True
...
def test_denies_caller_with_unrelated_business_unit_and_no_allowed_role():
    ...
    assert check_plus_access(ctx) is False
